...
build-docker-prod:
	docker build -t mattgleich/gex_slack:latest .
build-docker-dev:
	docker build -f dev.Dockerfile -t mattgleich/gex_slack:test .
build-docker-dev-lint:
	docker build -f dev.lint.Dockerfile -t mattgleich/gex_slack:lint .
build-go:
	go get -v -t -d ./...
	go build -v .
	rm gex_slack
